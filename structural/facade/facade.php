<?php
/**
* facade is a structural design pattern that allows objects with
* incompatible interfaces to collaborate.
*/
 namespace Structuralfacade{
  class FacadePattern{
    public function __construct(){
         add_action('wp_ajax_facadeClientCode', array($this,'facadeClientCode'));
         add_action('wp_ajax_nopriv_facadeClientCode', array($this,'facadeClientCode'));
    }
   
  public function facadeClientCode(){
   $title_type=$_POST['title_type'];
   $book = new Book('Design Patterns', 'Gamma, Helm, Johnson, and Vlissides');
   $thtml='showing title : ';
   $thtml.='Original book title: '.$book->getTitle();
  if($title_type!=1){
    $bookTitleReversed = CaseReverseFacade::reverseStringCase($book->getTitle());  
   $thtml.='<br>Reversed book title: '.$bookTitleReversed;
  }
